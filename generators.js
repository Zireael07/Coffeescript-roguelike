...
var generate_item, generate_npc, generate_prop;
...
generate_npc = function(m_id) {
  ...
  // equip equipment
  equip_list = [];
  if ('equipment' in State.npc_data[m_id]) {
    ref = State.npc_data[m_id]['equipment'];
...
generate_prop = function(_id) {
  ...
  // paranoia
  if (!(_id in State.props_data)) { // object presence
    console.log("Don't know how to generate " + _id + "!");
    return;
  }
  comps = [];
  ...
  if ('blocks_tile' in State.props_data[_id]) {
    comps.push(new TileBlocker());
  }
  if ('blocks_visibility' in State.props_data[_id]) {
